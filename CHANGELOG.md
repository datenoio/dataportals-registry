...
- MINERVA (`minerva`) software definition for LCSB SBGN pathway-map servers; remapped **16** Luxembourg and partner maps from `custom`.
- WebMain (`webmain`) software definition for Taiwan DGBAS-family `webMain.aspx` statistical query UIs; remapped existing tenants and added FOFA finds (Veterans Affairs Council, Control Yuan, Ministry of Civil Service, National Police Agency, Taipei TSIS); **12** catalogs.
- LORIS (`loris`) software definition for McGill MCIN longitudinal neuroimaging portals; remapped **7** study catalogs from `custom`.
- iMonitoring (`imonitoring`) software definition for NPO Krista Open Budget / `ifinmon.ru` portals; remapped **6** Russian regional catalogs from `custom`.
- IHK-Fachkräftemonitor (`ihkfachkraeftemonitor`) software definition for GWS multi-Land skills dashboards; remapped **5** Land tenants from `custom`.
- DUVA (`duva`) software definition for KOSIS-Gemeinschaft municipal Informationsportal; remapped **4** German city catalogs from `custom`.
- JAXI (`jaxi`) software definition for INE-ceded PC-Axis table browsers (`/jaxi/Tabla.htm`, IAEAxi, IBESTAT); remapped Aragón and Balearic tenants and added the SES crime-stats FOFA find (**3** catalogs).
- MATS (`mats`) software definition for Statistikamt Nord / Rheinland-Pfalz / Berlin-Brandenburg MATS-Datenportal; remapped **3** Land catalogs from `custom`.
- Nextstrain (`nextstrain`) software definition for pathogen phylodynamics hubs; remapped **3** catalogs from `custom`.
- Shanoir (`shanoir`) software definition for Inria Empenn medical-imaging archives; remapped **3** catalogs from `custom`.
- Health Data Center (`hdc`) software definition for Thailand MOPH `hdc.moph.go.th` warehouses; remapped the national hub and added the DMS FOFA find (**2** catalogs).
- ProteoSAFe (`proteosafe`) software definition for UCSD CCMS GNPS/MassIVE; remapped **2** catalogs from `custom`.
- CyVerse Data Commons (`cyverse`), ChecklistBank (`checklistbank`), OpenKIM (`openkim`), Materials Cloud (`materialscloud`), OpenSpending (`openspending`), Gharysh Geoportal Platform (`gharyshgeoportal`), and KAZGISA OpenLayers WebGIS (`kazgisaopenlayers`) software definitions; remapped the matching `custom` rows (one catalog each). Distinct from KAZGISA RGIS (`rgis`).
- FROST-Server (`frostserver`) software definition for Fraunhofer IOSB OGC SensorThings API servers (`/v1.1/` JSON with Things/Datastreams, or HTML `Start Page` / `FROST-Server`); added **8** public instances (Hamburg IoT, BGS, TLUBN groundwater, RIVM Samen Meten, New Mexico Bureau of Geology wells, NMED drinking water, Georgia Tech sea-level, BRGM water quality).
- DLCM (`dlcm`) software definition for the swissuniversities OAIS research-data platform; remapped Yareta and the inactive OLOS access portal from `custom`. `olos.swiss` stays `custom` (WordPress marketing homepage).
- easydb (`easydb`) software definition for Programmfabrik easydb 5 / fylr; remapped HilData and heidICON from `custom`.
- brain-GeoCMS (`braingeocms`) software definition for brain-SCC municipal GeoCMS; remapped Nordhessen and Vogelsbergkreis geoportals from `custom`.
- CodaLab Competitions (`codalab`) and Codabench (`codabench`) software definitions for the first-party ML competition hubs; remapped `competitions.codalab.org` and `www.codabench.org` from `custom`.
- DIGITAL TWIN CLOUD (`digitaltwincloud`) software definition for EGIS / NEWLAYER 3D GIS SaaS (HTML comment `DIGITAL TWIN CLOUD - NEWLAYER`, `assets/css/IDE.css`); remapped Ulsan Smart Map from `custom`.
- AddSpatial (`addspatial`) software definition for Icebound web GIS (title `AddSpatial`, `/smart/?profile=`); remapped Falkenberg municipal karta from `custom`.
- GeoNature (`geonature`), web-mapviewer (`webmapviewer`), DataHub (`datahubproject`), Hugging Face (`huggingface`), OpenAlex (`openalex`), Wikibase (`wikibase`), DBpedia Databus (`databus`), MGnify (`mgnify`), MetaboLights (`metabolights`), BioStudies (`biostudies`), Reactome (`reactome`), WikiPathways (`wikipathways`), UCSC Genome Browser (`ucscgenomebrowser`), FlyBase (`flybase`), WormBase (`wormbase`), iDigBio (`idigbio`), and iNaturalist (`inaturalist`) software definitions from the second custom-catalog OSS pass; remapped the matching `custom` rows, retagged CuGenDB to `tripal`, and added **14** French GeoNature-atlas instances from the PnX-SI named list.
...
- Add harvest-documented `apidetect` maps for Data Fair, QGIS Server, deegree, Micka, LKOD, Hugging Face, OpenAlex, and other hub APIs that use `absolute_url`; move software recipes that sat after `## Related` back above that heading.
- Add SuperMap iPortal, DaCHS TAP, DiVA/Dialnet OAI, Our Open Data, data eye, Databus, Reactome, and geo.admin layersConfig probes; expand leftover non-viewer harvest Keep/Drop recipes.
- Add `apidetect` URL maps for Hajk, CLLD, MINERVA, terGIS, GeoNature, Geocortex, ActiveMapGIS, Aubrey, GRIN-Global, Synapse, Yoda, DataONE, Open Data Cube, and OpenGov; remove those IDs from `NO_STANDARD_PROBE` where skip reasons were stale. Live `detect-software --action update` wrote matching endpoints onto catalogs that answered: GeoNature **28**, Geocortex **42**, OpenGov **4**, Open Data Cube **4**, DataONE **6**, Hajk **2** (`/publik/appConfig.json`). G3W-SUITE v4 public `/api/` paths 404; CLLD, MINERVA, terGIS, ActiveMapGIS, Aubrey, DataHub, GRIN-Global, Synapse, and Yoda already had the same endpoints or did not answer. Geocortex now probes only mixed-case `/Geocortex/Essentials/REST/sites?f=pjson`; GeoNature also tries `/atlas` when the catalog link is the origin.
- Add Origo `/index.json` probe (viewer config JSON) and origin cleanup so ATM Maggioli, ODWeb, DUVA, Beyond 20/20, TR32DB, and CodaLab harvest paths attach at the catalog origin. Live `action=update` wrote endpoints on Origo **18** (gallery landings Ånge and Sundsvall plus earlier viewers), ATM Maggioli **21**, Beyond 20/20 **3**, ODWeb **2**, CodaLab **1**, and DUVA **1**. Origo also reads `Origo('*.json')` from the viewer HTML, `{mapdir}/index.json` from gallery landing hrefs, and probes `/origo/index_ssl.json`. HUBzero `/resources?sortby=date` filled **8** catalogs; DGBAS Web **2**; IBIS-PH Utah **1**. Swing `/databank` and `/viewer/` filled **184** catalogs. JAXI has no anonymous list API and is on `NO_STANDARD_PROBE`.
- Add harvest-documented probes for GIN repos search, OpenDataSoft Explore 2.1, Socrata catalog v1, Junar/Drupal/DataPress/DHIS2/Knoema extras, HUBzero, DGBAS Web, Swing, and IBIS-PH; move the Galaxy `/api/libraries` recipe back under Galaxy (it had landed under Datasets lists).
- Add harvest Keep/Drop on leftover scientific, geoportal, earth-observation, biodiversity, and indicator recipes; extra probes for Stat Technology SDMX, WIS2 Box `/collections`, ESA TAP tables, RAMADDA JSON, CONTENTdm digital OAI, and Symbiota collections HTML.
- Move Keep/Drop that landed after the wrong heading (Data VAVT, Beyond 20/20) back into those recipes; add documented list GET URLs where harvest prose already named a relative path (NextGIS Web `/api/resource/`, hale»connect CSW, CoGIS REST, SuperWEB2 `/webapi/rest/v1/schema`, GIS Open Data Portal LKOD, and related IDs). Viewer one-liners still point at `harvest-viewers.md`.
- Add harvest-documented `apidetect` maps for SuperWEB2 `/webapi/rest/v1/schema`, Data Cube OWS `/collections`, CodaLab `/competitions/`, and HTML list paths already in harvest recipes (ATM Maggioli, ODWeb, SIMAI `/datasets/`, DUVA Informationsportal, Beyond 20/20 report folders, TR32DB). Skip Codabench, Goal Tracker, BRAHMS Online, and IBDC (hub/placeholder paths). Refresh source YAML counts (**467** software, **94** scheduled).
- Record remaining unmapped `software.id` values on `NO_STANDARD_PROBE` (map viewers plus BI/query UIs with no portable list API) so every named ID is mapped or skipped; CI asserts that split.
- Add Keep/Drop to leftover `harvest-viewers.md` recipes (layer/theme grain; drop tiles, print PDFs, basemaps, and login walls).
- Add Keep/Drop on leftover `harvest-geoportals.md` headings (viewer pointers plus QWC2, mviewer, Mapp Enterprise, and EliteGIS).
- Add **7** GeoViewer Online public map tenants (Huntington Park, Upland, Laguna Hills, Stanton, Moorpark, Los Alamitos, Hi-Desert Water District). CartoVista and NAZCA vendor/FOFA/crt.sh passes found no further public catalogs (Floridablanca timed out).
- Discovery and harvest guides now require a unique `## Name (`id`) {#id}` heading for every published `software.id` except `custom` (CI: `tests/test_docs_software_coverage.py`). Leftover “Other platforms” table rows were promoted to full recipes.
- Document `MISSING_CONTACT_INFO` as missing `owner.link` on active catalogs with restricted access.
- Document FOFA as the Censys alternative for catalog discovery: query translation (`title=` / `body=` / `host=` / `country=`), agent API usage, and example hunts (CKAN, OpenDataSoft, ODWeb, GeoNetwork, Dataverse, Hyrax, Tianditu, Wagmap).
- Custom-software review retagged leftover catalogs onto existing products: **5** Korean LifeMaps to `gtmap` (Gyeongju, Seongnam, Siheung, Gimpo, Gwangyang), **5** Jeolla living-information GIS to `myeongji` (Iksan, Gimje, Namwon, Jeongeup, Naju), **3** SHK city guides (`shkkbs`), **2** Berry GIS tenants (`berryict`), **2** Flood Intelligence Portal tenants (`floodintelligenceportal`), **2** ArcGIS Dashboards, **1** ArcGIS StoryMaps, and INDEC SIES to `shiny`. Indonesian `satudata.*` hostnames remain mixed stacks (Next.js, WordPress, Laravel, SI-WARTA), not a single software id.
- Second custom-software pass remapped Pohang, Wonju, and Jeonju LifeMaps to `gtmap`; Södertälje to `origo`; Sollentuna `/bios/dpwebmap/` to `dpwebmap`; Norrtälje to `mycarta`; Lund Filter Gallery to `instantapps`; and Yopal / Sabanalarga to `xiltrion`. Taiwan `webMain.aspx` stats UIs, `sdg.` hostnames, Indonesian `satudata.*`, and unnamed Korean `/map/main.do` shells stay `custom`.
- Third custom-software pass remapped leftover catalogs onto existing IDs: Düren `inkasportal`, Fulda and Darmstadt-Dieburg `kcwebgis`, Odenwaldkreis `weboffice`, Nová Paka `marushka`, Armenia and Soacha `nazca`, CARDER and Corpoboyacá `geoambiental`, and Yongin `gtmap`. Wałbrzych `gis.walbrzych.pl` stays `custom` (WebEWID-branded e-usługi hub, not Portal Mapowy).
- Fourth custom-software pass remapped leftover catalogs onto existing IDs: Drobeta and Giurgiu `avanmap`, Gulbene and Jēkabpils `pgis`, Ruapehu `infomap`, Mission Viejo `geoviewer`, Koper `3map`, Prostějov `marushka`, Uherské Hradiště `gisplan`, Forêt ouverte `igo2`, and the PHS / ScotPHO Shiny indicator apps `shiny`. PolarWatch `catalog/list.php` stays `custom` (ERDDAP is already a separate record). Bucheon and Busan LifeMaps stay `custom` (SODA probe failed).
- Fifth custom-software pass remapped leftover catalogs onto existing IDs: University of Guam Specify portal `specify`, Sir Arthur Lewis Community College Greenstone 3 library `greenstone`, and Géo Val-de-Marne `onegeosuite` (`data-theme="onegeo"`). WormQTL `molgenis.do` stays `custom` (live 403). TwinMap `{city}.twinmap.ai` open-data shells stay `custom` (Slovak od-portal template, not LKOD). MATS Länder pages stay `custom` (TYPO3 landings, not a shared app URL).
- Sixth custom-software pass remapped leftover catalogs onto existing IDs: ESIMO central and AARI/FERHRI regional nodes `esimo`, ASTRON VO `dachs` (`gavo_dc.css` / `gavo.js`), and the Finnish Cancer Registry plus Statistics Estonia labour-market apps `shiny`. UMCG research catalogue stays `custom` (live 403). `www.lovd.nl` stays `custom` (LOVD network page, not an installation).
- Seventh custom-software pass remapped leftover catalogs onto existing IDs: Neckar-Odenwald-Kreis and Hohenlohekreis `terratwin` (title `TerraTwin`, `alkis-*.js`), and the Mauritius Herbarium `brahmsonline` (title `BRAHMS: Mauritius Herbarium online`). ArVO stays `custom` (TAP host timed out). Belgian Health Data Agency catalog stays `custom` (live DataHub / `urn:li:` URNs, not MOLGENIS). BAW INGRID stays `custom` (InGrid chrome, but `ingrid` would force Geoportal). GeoFind Namibia stays `custom` (title Metadata Browser without MetadataWorks).
- Custom-catalog OSS review remapped PsychArchives to `eprints`, cBioPortal Cell Explorer to `cbioportal`, and Odisea Regional to `redatam`. BEXIS Jena and GAVO DaCHS nodes were already on those IDs. Materials Cloud Archive stays `inveniordm`.
- Second custom-catalog OSS pass remapped leftover named hubs onto new IDs (Hugging Face, OpenAlex, Wikidata/Wikibase, Databus, EMBL-EBI MGnify/MetaboLights/BioStudies, Reactome, WikiPathways, UCSC Genome Browser, FlyBase, WormBase, iDigBio Portal, iNaturalist, GeoNature-atlas, map.geo.admin.ch web-mapviewer, Belgian HDA DataHub) and CuGenDB onto existing `tripal`. `www.dbpedia.org` stays `custom` (Association WordPress homepage). MassBank Human / MB-POST, GBIF Norway Vortex CMS, DEIMS, CESSDA, Kaggle, and Open SDG hostnames stay `custom`. GeoNature-atlas URLs that timed out or 303'd (Vanoise, Pays de la Loire, SHF, LPO AURA carto, LR odonates) were not added.
- Eighth custom-software pass remapped leftover catalogs onto existing IDs: DataSud, PIGMA, and Data Grand Lyon `onegeosuite`; DatARA `prodige`; Rennes RUDI `rudi`; Tvrdošín and Nové Mesto TwinMap shells `gisopendataportal`; and the University of La Rioja research portal `dialnetcris`. UNT Data Repository stays `custom` (Aubrey host timed out).
- Ninth custom-software pass remapped leftover catalogs onto existing IDs: Lund Humanities Lab Archive `flat` (`flat_bootstrap_theme`, `/flat/oai2`); Royal Greenwich, Oxfordshire, Kingston, and Ealing `esridataobservatory` (`ia-theme`, Esri UK credit); and Oxford Brookes RADAR `openequella` (`com.equella.core`). UNT Digital Library stays `custom` (host still timed out). The Bashkortostan SIMAI demo stays `custom` (vendor placeholder, not a production portal).
- Tenth custom-software pass remapped leftover catalogs onto existing IDs: GAVO Data Center aliases `dachs` (`Server: DaCHS/2.12.2`, `gavo_dc` / `gavo.js`); SNSB DWB BioCASe pipeline `diversityworkbench`; Gaia@AIP, CARS, MUSE-Wide, RAVE, APPLAUSE, CosmoSim, and CLUES `daiquiri` (“Proudly powered by Daiquiri”); Biodiversity Exploratories `bexis2` (`/home/Start`); and eNanoMapper `ambit` (`meta name="description" content="AMBIT"`). DEB Register and CHD7 stay `custom` (live 403). GeoNature-atlas Guyane stays `custom` (no `geonature` software id). Belgian HDA DataHub stays `custom` (no `datahubproject` software id).
- Eleventh custom-software pass remapped leftover catalogs onto existing IDs: INFOMED national and Artemisa thesis catalogs `cwis` (CWIS JavaScript / `scout.wisc.edu/cwis`); and IRI/LDEO Climate Data Library `datalibrary` (title `Climate Data Library`, maproom). The IRI record was moved from Open data portal to Geoportal to match the software map. Liechtenstein `service.geo.llv.li` stays `custom` (CMS hub; GeoMapFish is `map.geo.llv.li`). `map.geo.admin.ch` stays `custom` (web-mapviewer successor, not mf-geoadmin3).
- Twelfth custom-software pass remapped leftover catalogs onto existing IDs: Nitra `gis.nitra.sk` `experiencebuilder` (iframe to `experience.arcgis.com/experience/`, live title `Experience` / `jimu-core` / `exb.ico`); 14 Czech city-domain GIS4U galleries `gis4u` (`/mapa/` apps plus `/theme/square/` and a tmapy.cz credit); and Choceň `marushka` (`/marushka/default.aspx`). NextGIS Drupal `ooptaari.nextgis.ru` stays `custom` (`/api/resource/` 404). Spanish CMS portals with mixed CKAN/Socrata/OpenDataSoft endpoint types stay `custom` (`status_show` empty). Białystok `gisbialystok.pl` stays `custom` (CMS hub linking Web AppBuilder and Portal Mapowy). Murcia `geoportal.murcia.es` stays `custom` (Drupal hub linking SITAM Experience Builder apps).
- Thirteenth custom-software pass remapped leftover catalogs onto existing IDs: Havířov and Pardubice GEOVAP MyCity galleries `marushka` (`/MarushkaPublic/default.aspx`; `/MarushkaGP4/` title `Marushka - Mapový aplikační server` / `zipped.js`); Mendel University repository `dspace` (`/server/api` reports DSpace 9.1); and Oradea `geomediawebmap` (`Intergraph.WebSolutions`, `WebClient.ashx`, `$GP`). ASEP dataset-info CMS stays `custom` (Preprints menu, not EPrints). Kadaň `/portal/` stays `custom` (`GeorealKadan` theme without `Georeal.Cards`). Trenčín `gis.tsk.sk` stays `custom` (ArcGIS Web ADF `mapviewer.jsf`, not Hub/ExB).
- Fourteenth custom-software pass remapped leftover catalogs onto existing IDs: University of Ostrava EDUO, Universitas Medan Area, and UNESP `dspace` (`/server/api` DSpace 7.6 / 10.0; DSpace 6.3 `generator` + JSPUI); Molecular Biophysics Database `inveniordm` (`generator` Invenio, `/api/records`); and Aleia, Deposita Dados, and Repo4Cat `dataverse` (`/api/info/version`). EOSC CZ National Metadata Directory stays `custom` (live InvenioRDM, but `inveniordm` cannot keep `catalog_type: Metadata catalog`). HilData stays `custom` (easydb 5, no software id).
- Fifteenth custom-software pass remapped leftover catalogs onto existing IDs: SADAR Halle `dspace` (`generator` DSpace 6.4); HCU Hamburg repOS `dspacecris` (`generator` DSpace CRIS-5.10.0); PTB OAR and Nülan UNMDP `eprints` (OAI `eprints` xmlns; `generator` EPrints 3.4.4); TUL open research data `dataverse` (`/api/info/version` 6.8); University of Wrocław repository `dlibra` (`/dlibra`); and eLTER DAR `invenio` (`generator` Invenio, `/api/records`, no InvenioRDM branding). Radboud Data Repository stays `custom` (no public Yoda landing). Lublin UP repository stays `custom` (not `/dlibra`).
- Sixteenth custom-software pass remapped leftover catalogs onto existing IDs: InDoRES `dataverse` (`/api/info/version` 6.3); Offenbach `mapbender` (`/mapbender/application/geoportal_offenbach`); Schaumburg BürgerGIS `qwc2` (`assets/css/qwc2.css`); Kronach 360° `touviamaps` (`loadTouviaMaps()`, `touvia.de` assets, vianovis copyright); and Landkreis Osnabrück WebInfo `weboffice` (`synserver`, `weboffice_modern_user.css`). Merzig-Wadern and Trier-Saarburg stay `custom` (CMS hubs linking Saarland/RLP Mapbender). Radolfzell Bürgerportal stays `custom` (CMS link to the existing VertiGIS host). Szczecin SIP stays `custom` (CMS hub; GeoNetwork is `/geonetwork`). Quetelet-Progedo stays `custom` (not NADA).
- Seventeenth custom-software pass remapped leftover catalogs onto existing IDs: SDIS 04 OpenSIS `lizmap` (`lizmapPopup` / dock CSS, `/index.php/view/`); and Géo Pays de Brest `arcgishub` (`/portal/apps/sites/` with `hub-site` / `opendata-ui`). Sondrio `?mapset=` stays `custom` (GisClient, not G3W-SUITE). Brest `/portal/home/` stays ArcGIS Enterprise Portal, not Hub. SITMUN and OpenSIS stay without new software ids (one leftover each).
- Eighteenth custom-software pass remapped leftover catalogs onto existing IDs: BAW HENRY `dspace` (`/server/api` DSpace 7.5); AERADE Cranfield `dspace` (`generator` DSpace 5.10); RWTH Publications `invenio` (JOIN2 Invenio, `/oai2d`); GALENOS `inveniordm` (`generator` InvenioRDM 12.0); Warwick WRAP `eprints` (`generator` EPrints 3.4.5); and Eisenbahn-Bundesamt GeoPortal `ingrid` (`/user/themes/ingrid/`, `ingrid.js`). BAW INGRID scientific stays `custom`. EUR library CMS stays `custom` (Dataverse mentioned, not the install). German TerraWeb stays `custom` (not Latvian `tergis`).
- Nineteenth custom-software pass remapped leftover catalogs onto existing IDs: Rice Research Repository `dspace` (`/server/api` DSpace 9.3); AuScope Data Repository `ckan` (`generator` CKAN 2.10.1, `status_show`); Lancaster datasets portal `pure` (“Powered by Elsevier Pure”); and RIT Digital Institutional Repository `elsevierdigitalcommons` (bepress Digital Commons chrome). ReDATA stays `custom` (library GitHub Pages linking `arizona.figshare.com`). Krakow UR `/dlibra` stays `custom` (Cloudflare challenge). Hull Hydra stays `custom` (redirects to the library CMS).
- Twentieth custom-software pass remapped leftover catalogs onto existing IDs: SJSU ScholarWorks `elsevierdigitalcommons` (Digital Commons / `dc-responsive`); UNAM IIEc `eprints` (`generator` EPrints 3.3.16, `/cgi/oai2`); and UNCo RDI `dspace` (`/server/api` DSpace 8.4). Claremont CCDL stays `custom` (CONTENTdm photo/manuscript library, not a dataset catalog). Stanford SDR stays `custom` (Drupal service hub). IGN `mapamuni` stays `custom` (not Argenmap).
- Twenty-first custom-software pass remapped leftover catalogs onto existing IDs: Città Metropolitana di Torino `geonetwork` (root redirects to `/geonetwork/cmto/ita/catalog.search`, live CSW); RSDI Basilicata `geonetwork` (`/geonetwork/srv/eng/catalog.search` Catalogo RSDI, live CSW); and uBibliorum `dspace` (`/server/api` DSpace 7.6.1). Liguria/Sicily/Lombardy/Veneto/Bergamo geoportal homes stay `custom` (`/geonetwork/srv` 404). UGM ETD stays `custom` (`/server/api` 404).
- Twenty-second custom-software pass remapped leftover catalogs onto existing IDs: GéoArdèche `geonetwork` (`/geonetwork/srv/eng/catalog.search`, 143 local records, live CSW); Atmo Nouvelle-Aquitaine `geonetwork` (branded `catalog.search`, live CSW); and Observatorio Medioambiental La Plata `ckan` (`generator` CKAN 2.7.3, live `status_show`). UBA Sociales stays `custom` (Omeka Classic `/items/browse`, not Omeka S and not Dataverse). Yareta stays `custom` (DLCM OAI, no software id). University of Macau Scholars stays `custom` (CSpace). SIG Cévennes `/index.php/view/` stays `custom` (CMS homepage, not Lizmap).
- Twenty-third custom-software pass remapped leftover catalogs onto existing IDs: Bloemendaal `arcgishub` (`hubcdn.arcgis.com/opendata-ui`, live `/api/search/v1` and DCAT-US feed); and Lille Métropole `geonetwork` (`gn_search_georchestra` `catalog.search`, live CSW). Ulm Daten stays `custom` (`/mapapps/` viewer, but `mapapps` would force Geoportal). Nordhessen and Vogelsberg stay `custom` (brain-SCC GeoCMS; no software id). Gera stays `custom` (GajaMatrix viewer; the vendor GeoNetwork catalog is already tagged). Spanish `/dataset` CMS portals stay `custom` (`status_show` 403/timeout).
- Twenty-fourth custom-software pass remapped leftover catalogs onto existing IDs: Knowledge@UChicago `inveniordm` (`generator` InvenioRDM 13.1, live `/api/records`); UCL Discovery, NERC Open Research Archive, LSE Research Online, and LSHTM Research Online `eprints` (`generator` EPrints 3.4.x, live `/cgi/oai2`); and Surrey Research Insight `esploro` (`/esploro/` after the old EPrints host redirected). EPA Maps GIS stays `custom` (maps landing; GeoNetwork is `/geonetwork` and is not this record). IDEE geoportal stays `custom` (CODSI GeoNetwork is already tagged). Italian Municipium “Portale Opendata” stays `custom` (civic CMS, not `atmmaggioli`). Goobi viewer digital libraries stay `custom` (heritage collections, no software id). `datarepository.stat.unipd.it` stays `custom` (Dataverse-like title without `/api/info/version`). INAIL open data stays `custom` (HTML “ckan”, `status_show` 503).
- Twenty-fifth custom-software pass remapped leftover catalogs onto existing IDs: UAL Research Online `eprints` (`generator` EPrints 3.4.5, live `/cgi/oai2`); GlobeData `dataverse` (`/api/info/version` 6.10.1); BBAW edoc `opus` (`Opus-Version` 4.8, OAI Identify `Opus4 BBAW Instance`); and WU Vienna Research `pure` (“Angetrieben von Pure”). Recode UCC stays `custom` (connection reset). EPA SAFER stays `custom` (captive-portal redirect). heidICON stays `custom` (easydb 5, no software id). Atlanta University Center RADAR stays `custom` (homepage 403).
